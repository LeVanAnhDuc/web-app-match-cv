...
ESLint starts from [tanstack/eslint-config](https://tanstack.com/config/latest/docs/eslint) and layers on `react`, `react-hooks`, `jsx-a11y`, `promise`, `unused-imports` and `prettier` so the rule set matches `server/` and the sibling app `web-app-ducker-id`. Prettier shares the same style as those projects (double quotes, semicolons, `trailingComma: none`, `printWidth: 80`) plus `prettier-plugin-tailwindcss` for Tailwind class sorting.
