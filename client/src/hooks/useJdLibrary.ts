...
/**
 * Page hook for the saved-JD library (`/jd`): owns the saved-JD query, the
 * per-row mutations (rename / delete / lineage) and the dialog state the
 * JdLibrary view renders. Deliberately JD-only and separate from
 * `useCvLibrary` so the two pages can diverge without touching each other.
 *
 * `contextHolder` must be rendered by the view — that is how antd's message
 * API attaches to the tree. No compare action: version comparison only
 * accepts a CV (docs/specs/cv-version-comparison/security-report.md).
 */
export function useJdLibrary() {
  const { t } = useTranslation();
  const [messageApi, contextHolder] = message.useMessage();

  const savedQuery = useSavedDocuments("JD");
